 ...
 *   - DEDUP  — an identical large block that appears again later is replaced by a marker
 *              (the latest full copy is kept, so nothing current is lost)
...
function compress(messages, opts = {}) {
  ...
  msgs.forEach((m, i) => {
    ...
    // truncate a big old block to head + tail (but never the surviving dedup copy)
    if (c.length > truncateOver && !keepers.has(i)) {
      m.content = truncateMiddle(c, headChars, tailChars);
      elided++;
    }
  });
