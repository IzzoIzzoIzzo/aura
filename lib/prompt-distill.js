...
function distill(prompt, opts = {}) {
  ...
  if (trimFiller) {
    const FILLER = /^(\s*(?:[-*+]|\d+[.)])\s+)?(?:please note that|it is important to (?:note|remember) that|it's important to|as (?:mentioned|noted) (?:above|earlier|before),?|remember to|make sure to|be sure to|note that|kindly)\s+/i;
    units.forEach((u) => {
